...
轮转也是**主动的**，而非只在失败时被动反应。每个 key 都携带一个 trailing `keyBalanceWindowMs` 窗口（默认五小时）内的近似请求计数，适配器偏好**使用最少的可选 key**，让流量在池中铺开，而不是集中到单个 key 上（其余 key 闲置时「只薅一只羊」）。被选中的 key 会**钉住** `keyStickyMs`（默认五分钟），之后轮转器才重新评估——钉住让某个 key 在一段内保持使用，使提供方的 per-key 提示缓存保持温热，因为每次请求都换 key 会打散该缓存并拉低命中率。窗口仍让轮转器周期性地再平衡，而非永不再平衡：钉住期过后它可能移向更安静的 key，而一次拒绝会立即清除钉住并离开被冷却的 key 再平衡。计数在窗口耗尽时整体清零（设计上即近似），因此几小时前忙碌的 key 不会继续把流量从它身上引开。两个旋钮都是按路由的 config 字段，默认值如上；单 key 路由不受影响。

### 按会话亲和

当一次请求点名了 session，该路由会把**该 session 钉到单个 key**（`keyPinBySession`，默认开启）。session 的首次请求选取负载最少的可选 key 并记录钉住；该 session 之后的每次请求都复用它。因为每个 session 的负载都计入其钉住的 key，并发 session 会在池中铺开，而不是全部猛捶一个 key——那正是触发单个 key 429 的原因——而单个 session 保持一个 key，使其 per-key 提示缓存保持温热。钉住持续 `keySessionPinMs`（默认三十分钟），若其 key 被冷却或钉住过期则释放，之后 session 再平衡到更安静的 key；没有 session id 的请求（或关闭了钉住）使用上面的全局「最少负载 + 钉住」选择。一次 key 拒绝仍会冷却该 session 用过的确切 key，并迫使下一次请求落到另一个 key 上。
...
成功响应的 finish 分片携带 `answeredBy`——实际应答的路由与模型。密钥池或跨提供方故障转移可能在请求点名的路由之外应答，而 loop 会以 `answeredBy` 为已记录的 assistant source 盖戳，因此回放状态旁边的 source 指名的正是捕获该状态的路由。`answeredBy` 缺席即表示由被请求的路由应答。

如果 listener 改写已组装 assistant 内容，loop 会在记录消息前丢弃回放状态，因为其提供方元数据不再描述该内容。回放状态记录的提供方或模型与消息 source 不一致时，会降级为外来投影——内容保留，pi-ai signature 连续性不保留——因为 finish 分片携带 `answeredBy` 之前写下的日志，会把被请求的路由记为 source，而故障转移实际在别处应答；此时让轮次失败会把会话永久困在一次过期的配对上。无效版本、格式错误元数据，以及内容／块不匹配仍会显式以 `LlmError('INVALID_REPLAY_STATE')` 失败。
...
- **模态声明不经验证，且多声明的后果超出本轮**：没有任何环节会去询问端点接受什么，因此声明了网关并不提供的 `image` 的模型不会在这里被拦下，而是由提供方在轮次中途拒绝。prompt 准入在构造请求之前就把用户消息持久化提交，于是被拒绝的图片留在会话日志里。此后对纯文本路由的请求会把该历史图片块降级为占位文本——日志仍保留图片及此前对它的任何分析——因此切换到纯文本模型就是恢复途径，而非死路。仍然拒绝的只剩已排队未发送的图片：inbox 中有图片等待时，纯文本路由的选择会被拒绝，因为它们会作为新输入发出、随即被该路由拒绝。
